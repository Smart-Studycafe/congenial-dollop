...
class Application:
    def __init__(self):
        ...
        col1 = [[sg.Column([
            ...
            ], size=(320, 320), pad=(0, 0))]], font=middle_font)], ], pad=(0, 0), element_justification='c')]]

        col2 = [[sg.Column([[sg.Frame('CAM {}'.format((row)), [
            [sg.Image(key='_SMARTSTUDY/STREAM/{}_'.format((row)), size=(480, 320))],
        ], font=middle_font) for row in range(1, 3)]], pad=(0, 0), element_justification='c')]]
    ...
    def aws_connect(self, client_id):
        ENDPOINT = "a2ijsbymx6vfow-ats.iot.ap-northeast-2.amazonaws.com"
        PATH_TO_CERT = "./certificates/CORE_CERT.crt"
        PATH_TO_KEY = "./certificates/CORE_KEY.pem.key"
        PATH_TO_ROOT = "./certificates/CORE_CA.pem"
        ...
        try:
            if self.myAWSIoTMQTTClient.connect():
                ...
                for i in range(1,3):
                    self.mqtt_subscribe('smartstudy/stream/{}'.format(i))
